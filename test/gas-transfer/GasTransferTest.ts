  ...
  testCases.forEach(function (testCase) {
    xit('Should run case ${testCase.name}', async function() {
...

      {
        ...
        if (DEBUG) {
          console.log(`[0] balance was ${accountBalance} we used ${totalGas} on gas, and gave the contract ${testCase.constructContractWith}`)
        }
        ...
        if (DEBUG) {
          console.log(`[2] Contract has ${contractBalance} of ${testCase.constructContractWith}; account has ${accountBalance}`)
      ...
      // Send some money to the contract
      {
        ...
        accountBalance = await provider.getBalance(myAddress);
        if (DEBUG) {
          console.log(`[2.5] transfer in ${testCase.transferIntoContract} => contract now has ${contractBalance}, account ${accountBalance}, gas ${totalGas}`);
        }
        ...
        if (DEBUG) {
          ...
          console.log(`[2.5] contract expected balance ${testCase.expectedContractAfterTransferIn}, actual ${contractBalance}`)
      ...
      // Now send it back
      {
        ...
        accountBalance = await provider.getBalance(myAddress);
        if (DEBUG) {
          console.log(`[2.75] transfer out ${testCase.transferOutOfContract} => contract now has ${contractBalance}, account ${accountBalance}, gas ${totalGas}`);
        }
        ...
        if (DEBUG) {
          console.log(`[2.5] expected balance ${expectedBalance}, actual ${accountBalance}`)
          console.log(`[2.5] contract expected balance ${testCase.expectedContractAfterTransferOut}, actual ${contractBalance}`)
          console.log(`[2.5] I won ${won}`);
        }
