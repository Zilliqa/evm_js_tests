...
describe("Contract Interaction with web3.js", function () {
  describe("When public setter function is called w3", function () {
    ...
    xit("Should set uint256 internal state correctly with external method", async function () {
      const TO_BE_SET = 100;
      expect(await this.contract.methods.setNumberExternal(TO_BE_SET).send()).to.be.not.null;
      expect(await this.contract.methods.number().call()).to.be.eq(web3.utils.toBN(TO_BE_SET));
    });

    xit("Should set string internal state correctly", async function () {
    ...
    xit("Should set string internal state correctly with external method", async function () {
      const TO_BE_SET = "Zilliqa";
      expect(await this.contract.methods.setNameExternal(TO_BE_SET).send()).to.be.not.null;
      expect(await this.contract.methods.name().call()).to.be.eq(TO_BE_SET);
    });

    xit("Should set enum internal state correctly", async function () {
      const ENUM = "1";
    ...
    xit("Should set address internal state correctly", async function () {
    ...
    xit("Should have event in returned object when no arg is specified", async function () {
